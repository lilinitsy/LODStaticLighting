...
// Sets default values
AExperimentalSetup1::AExperimentalSetup1()
{
...
void AExperimentalSetup1::BeginPlay()
{
	Super::BeginPlay();
}

// Called every frame
void AExperimentalSetup1::Tick(float DeltaTime)
{
	...
	if(timerinfo.runtimer)
	{
		...
		float lodpop_time = 2.0f;
		if(timerinfo.dt >= lodpop_time)
		{
...
void AExperimentalSetup1::SetupPlayerInputComponent(UInputComponent *PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);
	PlayerInputComponent->BindAction("ExecuteTrial", IE_Released, this, &AExperimentalSetup1::execute_trial);
}


UStaticMeshComponent *AExperimentalSetup1::find_actor_static_mesh_component(TArray<UStaticMeshComponent *> static_mesh_components, AActor *actor)
{
...
void AExperimentalSetup1::execute_trial() // ExperimentalParams param)
{
	if(initialized)
	{
		...
		if(lodpop_actor != nullptr && control_actor != nullptr)
		{
			...
			{
				control_actor->SetActorLocation(FVector(100.0f, 100.0f, 64.0f));
				lodpop_actor->SetActorLocation(FVector(100.0f, -100.0f, 64.0f));
			}

			else // lodpop on the right
			{
				control_actor->SetActorLocation(FVector(100.0f, -100.0f, 64.0f));
				lodpop_actor->SetActorLocation(FVector(100.0f, 100.0f, 64.0));
			}
		...
		// FPlatformProcess::Sleep(4.0f); --> This won't work, it just freezes the thread and code before it too...
...
void AExperimentalSetup1::increment_lod(UStaticMeshComponent *lod_object, const bool share_lightmap) //, TStructOnScope<FActorComponentInstanceData> &component_instance_data)
{
...
void AExperimentalSetup1::set_experiment_params()
{
...
ModelParams AExperimentalSetup1::pick_model_params(ModelID model_id)
{
	// Adjust scales later
	ModelParams params;
	params.id = model_id;
